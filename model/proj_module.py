...
class ProjNet_contrast(nn.Module):#目前就用FC做特征映射，改变特征分布，不用其他结构
    def __init__(self,
                in_dim,out_dim,
                hid_dim_list,
                activate_func='relu',
                ...
                dropout_fc=0):
        super(ProjNet_contrast, self).__init__()
        #多少隐层
        ...
        layer=nn.Sequential(
                ...
                nn.BatchNorm1d(num_features=out_dim),#0604 对proj后特征都做归一化
        )
...
class ProjNet_mask(nn.Module):#目前就用FC做特征映射，改变特征分布，不用其他结构
    def __init__(self,
        ...
        super(ProjNet_mask, self).__init__()
        #多少隐层
        hid_layer_num=len(hid_dim_list)
        # 选择激活函数--------------------------------------------------
        if activate_func == 'relu':
        ...
        self.reweight_layer_electrode = nn.Sequential(
        ...
        self.reweight_layer_time = nn.Sequential(
            # 简单整合所有电极信息，forward中再调整数据维度顺序
            nn.Conv2d(in_channels=1, out_channels=timepoint_num,
                      kernel_size=(timepoint_num, 1), stride=(1, 1)),
        )
    ...
    def forward(self, in_data):
        ...
        reweighted_data = self.reweight_layer_electrode(in_data_unsqueeze)
        ...
        reweighted_data = self.reweight_layer_time(reweighted_data)
        # batch 32 1 124

        # print('reweighted_data',reweighted_data.shape)


        cls_token=reweighted_data
        # 拉成1维，接fc
        fc_in = cls_token.reshape(cls_token.shape[0], -1)
        # print('fc输入特征的维度大小:', fc_in.shape[1])
        fc_out = self.model(fc_in)
